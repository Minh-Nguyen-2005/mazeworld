...
class Maze:
    ...
    # internal structure:
    #   self.map: list of location in map from top to bottom, left to right (contains . # only)
    #   self.width: number of columns
    #   self.height: number of rows
    #   self.robot_locations: store x,y coordinates of robots, in the form [x, y, x, y, x, y, ...]
    ...
    def __init__(self, mazefilename):
        ...
        for line in f:
            ...
            if len(line) == 0:
                pass
            # command \robot adds a robot at a location (x,y)
            elif line[0] == "\\":
                #print("command")
                ...
                x = int(parameters[1])
                y = int(parameters[2])
                self.robot_locations.append(x)
                self.robot_locations.append(y)
            # a row in the maze
            else:
                lines.append(line)
        f.close()
    ...
    # Get index of a location (x,y) in map list
    def index(self, x, y):
    ...
    # returns True if location has a robot
    def has_robot(self, x, y):
        ...
        for i in range(0, len(self.robot_locations), 2):
            rx = self.robot_locations[i]
            ry = self.robot_locations[i + 1]
            if rx == x and ry == y:
                return True
    ...
    def create_render_list(self):
        ...
        robot_number = 0
        for index in range(0, len(self.robot_locations), 2):

            x = self.robot_locations[index]
            y = self.robot_locations[index + 1]
...
# Assign a character from A-Z for robot
...
if __name__ == "__main__":
    ...
    print(test_maze3.robot_locations)
